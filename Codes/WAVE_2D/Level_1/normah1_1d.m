function [err_h1]=normah1_1d(fdq, nq, errtype, u, uex, uexx,...
 x, wx, dx, xx, jacx, xy,nov)
% NORMAH1_1D   Computes H1-norm in 1D
%
%  [err_h1]=normah1_1d(fdq, nq, errtype, u, uex, uexx,...
%            x, wx, dx, xx, jacx, xy,nov)
%
...
%         uexx  = first derivative of exact solution 
%                 (uexx=@(x)[uex(xx)], with .*, .^, ./)
...
[norma_loc1,norma_loc2]=normah1_ex_loc(errtype,npdx,uex,uexx,...
 u_loc,x,dx,jacx(ie),xx(1:2,ie),xg,wxg,nq,phix);
num=num+norma_loc1;
den=den+norma_loc2;
end
...
[norma_loc1,norma_loc2]=normah1_loc(errtype,npdx,uex,uexx,...
 u_loc,xy_loc, wx,dx,jacx(ie));
num=num+norma_loc1;
den=den+norma_loc2;
end
...
function [norma_loc1,norma_loc2]=normah1_ex_loc(errtype,npdx,uex,uexx,...
 u, x,dx,jacx,xx,xg,wxg,nq,phix);
...
norma_loc1=sum(((U-u_i).^2+(UX-ux_i).^2).*wxg)*jacx;

if errtype==0
    norma_loc2=0;
else
    norma_loc2=sum((U.^2+UX.^2).*wxg)*jacx;
end

return

function [norma_loc1,norma_loc2]=normah1_loc(errtype,npdx,uex,uexx, u_loc,...
 xy_loc, wx,dx, jacx); 
...
norma_loc1=sum(((U-u_loc).^2+(UX-ux).^2).*wx)*jacx;
if errtype==0
    norma_loc2=0;
else
    norma_loc2=sum((U.^2+UX.^2).*wx)*jacx;
end
